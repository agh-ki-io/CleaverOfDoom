 ...
 */
public class BarbarianMeleeAttackSkill extends Skill {
    private final StaticShapedAttack attack;
    //o ile od srodka gracza w danym kierunku przesunac srodek ataku. (pixele * skala)
    private static final float DELTA_MULTIPLIER = 4 * 5;

    public BarbarianMeleeAttackSkill(Level level, Character skillUser) {
        super(level, skillUser);
        attack = new StaticShapedAttack(skillUser.getX(), skillUser.getY(), 5 * 6, new Damage(DamageType.PHYSICAL, 500), 1, 0.2f);
